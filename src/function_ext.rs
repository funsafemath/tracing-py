use pyo3::{
    Bound,
    ffi::{PyFunction_GetCode, PyFunction_GetDefaults, PyFunction_GetKwDefaults},
    types::{PyCode, PyDict, PyFunction, PyTuple},
};
...
impl<'py> PyFunctionMethodsExt<'py> for Bound<'py, PyFunction> {
    ...
    fn code(&self) -> Bound<'py, PyCode> {
        // always returns a valid borrowed pointer, https://docs.python.org/3/c-api/function.html#c.PyFunction_GetCode
        unsafe {
            Bound::from_borrowed_ptr(self.py(), PyFunction_GetCode(self.as_ptr()))
                .cast_into_unchecked()
        }
    }
}
