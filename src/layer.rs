...
pub(crate) fn py_init(py: Python<'_>, layers: Option<Bound<'_, PyAny>>) -> PyResult<()> {
    ...
    let guard_vec = guard_vec.into_pyobject(py)?;

    let closure = match PyCFunction::new_closure(py, None, None, {
        let guard_vec = guard_vec.clone().unbind();
        ...
        Err(err) => {
            ...
            guard_vec.borrow_mut().drop_guards();
            return Err(err);
        }
    };
    match get_atexit_register(py).call1((&closure,)) {
        Ok(_) => Ok(()),
        Err(err) => {
            // same
            guard_vec.borrow_mut().drop_guards();
