...
impl PyTimer {
    ...
    #[new]
    #[pyo3(signature = (time = Time::Utc, format = Python::attach(|py| Py::new(py, PyTimeFormat::ISO_8601).unwrap().into_any())))]
    fn new(py: Python<'_>, time: Time, format: Py<PyAny>) -> PyResult<Self> {
