pub(super) mod generator;
mod parameter;
mod signature;

use std::{collections::HashMap, sync::LazyLock, time::Instant};

use eyre::{ContextCompat, bail};
use pyo3::{
    exceptions::PyTypeError,
    prelude::*,
    types::{PyCFunction, PyDict, PyFunction, PyList, PyString, PyTuple},
};
use tracing::{Level, error, error_span};

use crate::{
    function_ext::PyFunctionMethodsExt,
    imports::{get_inspect_signature, get_wrapt_decorator},
    instrument::{
        parameter::{ParamKind, PyParameter, PyParameterMethods},
        signature::{PySignature, PySignatureMethods},
    },
    leak::{Leaker, VecLeaker},
    span::span,
};

pub(super) static INSTRUMENT: LazyLock<Py<PyAny>> = LazyLock::new(|| {
    Python::attach(|py: Python<'_>| {
        get_wrapt_decorator(py)
            .call1((PyCFunction::new_closure(py, None, None, instrument).unwrap(),))
            .unwrap()
            .into_any()
            .unbind()
    })
});

// a simplified function signature struct; todo: support default values
// binding algorithm:
// create an array of size [arg cnt + kwarg cnt (excludes *args, **kwargs))]
// for each arg, set array[i] to arg if i < num of positional args, otherwise
//  if excess_args: push arg to args
//  else: return an error
// for each kwarg, set array[index(kwarg)] to kwarg, if index is already set, return an error,
// if index does not exist:
//  if excess_kwargs: push kwarg to kwargs
//  else: return an error
// apply defaults
// if not all args are set, return an error, othervise the signature is bound
// todo: it is broken if the kwarg names are invalid unicode;
// make the code always return the error on invalid unicode instead of silently doing potentially incorrect things
#[derive(Debug)]
struct Signature {
    // using str may lead to a bug in subscriber if two invalid-unicode params normalize to the same unicode strings
    // (afaik json subscriber will emit a field twice, fmt will subscriber print it twice)
    param_names: &'static [&'static str],
    kwarg_to_index: HashMap<&'static str, usize>,
    has_excess_args: bool,
    has_excess_kwargs: bool,
    pos_only: usize,
    pos_or_kw: usize,
    kw_only: usize,
}

impl Signature {
    fn has_excess_args(&self) -> bool {
        self.has_excess_args
    }

    fn has_excess_kwargs(&self) -> bool {
        self.has_excess_kwargs
    }

    fn kwargs(&self) -> HashMap<&'static str, usize> {
        HashMap::default()
    }

    // todo: binding can be done more efficiently in a single pass over 0..pos_only, 0..pos_or_kw and 0..kw_only
    // though it's already >40x times faster than inspect.Signature.bind
    fn bind<'a, 'py>(
        &self,
        py: Python<'py>,
        args: &'py Bound<'py, PyTuple>,
        kwargs: Borrowed<'a, 'py, PyDict>,
        defaults: Option<Bound<'py, PyTuple>>,
        kw_defaults: Option<Bound<'py, PyDict>>,
    ) -> eyre::Result<Vec<Bound<'py, PyAny>>> {
        // todo: move assert to the constructor
        assert!(
            self.param_names.len()
                == self.pos_only
                    + self.pos_or_kw
                    + self.kw_only
                    + self.has_excess_args as usize
                    + self.has_excess_kwargs as usize
        );

        let mut bound_args: Vec<Option<Bound<'py, PyAny>>> = vec![None; self.param_names.len()];

        // let mut excess_kwargs: HashMap<String, Bound<'py, PyAny>> = HashMap::default();
        let mut excess_kwargs: Vec<(Bound<'py, PyString>, Bound<'py, PyAny>)> = Vec::new();

        let mut args = args.into_iter();

        for (i, arg) in (&mut args).take(self.pos_only + self.pos_or_kw).enumerate() {
            bound_args[i] = Some(arg);
        }

        if !args.is_empty() && !self.has_excess_args() {
            bail!("too many positional arguments")
        }

        let excess_args: Vec<Bound<'py, PyAny>> = args.collect();

        for (kw_name, kw_value) in kwargs.iter() {
            let kw_name: Bound<'py, PyString> = kw_name.cast_into().unwrap();
            let index = self.kwarg_to_index.get(kw_name.to_str().unwrap());
            match index {
                Some(index) => bound_args[*index] = Some(kw_value),
                None => {
                    if self.has_excess_kwargs() {
                        excess_kwargs.push((kw_name, kw_value));
                    } else {
                        bail!("too many keyword arguments")
                    }
                }
            }
        }

        if self.has_excess_args() {
            let list = PyList::new(py, excess_args).unwrap();
            bound_args[self.pos_only + self.pos_or_kw] = Some(list.into_any());
        }

        if self.has_excess_kwargs() {
            let list = PyList::new(py, excess_kwargs).unwrap();
            let last = bound_args.len() - 1;
            bound_args[last] = Some(PyDict::from_sequence(&list).unwrap().into_any());
        }

        if let Some(defaults) = defaults {
            let defaults = defaults.into_iter().rev();
            let pos_args = &mut bound_args[..self.pos_only + self.pos_or_kw];

            for (arg, default) in pos_args.iter_mut().rev().zip(defaults) {
                if arg.is_none() {
                    *arg = Some(default);
                }
            }
        }

        if let Some(kw_defaults) = kw_defaults {
            let kw_defaults = kw_defaults.into_iter();

            for (kw_name, kw_value) in kw_defaults {
                let kw_name = kw_name.cast::<PyString>().unwrap().to_str().unwrap();
                if let Some(kw_idx) = self.kwarg_to_index.get(kw_name) {
                    let entry = &mut bound_args[*kw_idx];
                    if entry.is_none() {
                        *entry = Some(kw_value);
                    }
                }
            }
        }

        bound_args
            .into_iter()
            .collect::<Option<Vec<_>>>()
            .context("not enough arguments")
    }
}

struct BoundSignature<'py> {
    signature: &'static Signature,
    args: Vec<Bound<'py, PyAny>>,
    extra_args: Vec<Bound<'py, PyAny>>,
    kwargs: Vec<Bound<'py, PyAny>>,
}

// Builds a Signature using inspect.signature
// inspect.signature is actually not that slow for non-native functions,
// but we cache the signature anyway
fn extract_signature<'a, 'py>(func: Borrowed<'a, 'py, PyFunction>) -> PyResult<Signature> {
    let signature = get_inspect_signature(func.py())
        .call1((func,))?
        .cast_into()
        .map_err(|x| PyTypeError::new_err(x.to_string()))?;
    let params = signature.parameters();

    let mut param_names = Vec::new();
    let mut leaker = Leaker::acquire();
    let mut pos_only = 0;
    let mut pos_or_kw = 0;
    let mut kw_only = 0;
    let mut has_excess_args = false;
    let mut has_excess_kwargs = false;
    let mut kwarg_to_index = HashMap::default();
    let mut defaults = Vec::new();

    for (i, param_tuple) in params.items()?.into_iter().enumerate() {
        let param_tuple = param_tuple.cast::<PyTuple>()?;
        let param_name = param_tuple.get_item(0)?;
        let param_name = leaker.leak_or_get(param_name.str().unwrap().to_string());
        let param = param_tuple.get_item(1)?.cast_into::<PyParameter>()?;
        match param.kind() {
            ParamKind::PositionalOnly => pos_only += 1,
            ParamKind::PositionalOrKeyword => {
                pos_or_kw += 1;
                kwarg_to_index.insert(param_name, i);
            }
            ParamKind::ExcessArgs => has_excess_args = true,
            ParamKind::KeywordOnly => {
                kw_only += 1;
                kwarg_to_index.insert(param_name, i);
            }
            ParamKind::ExcessKwargs => has_excess_kwargs = true,
        }
        if param.has_default() {
            defaults.push(i);
        }
        param_names.push(param_name);
    }
    let param_names = VecLeaker::leak_or_get_once(param_names);

    Ok(Signature {
        param_names,
        kwarg_to_index,
        has_excess_args,
        has_excess_kwargs,
        pos_only,
        pos_or_kw,
        kw_only,
    })
}

fn instrument(
    args: &Bound<'_, PyTuple>,
    kwargs: Option<&Bound<'_, PyDict>>,
) -> PyResult<Py<PyAny>> {
    // wrapt passes 4 positional arguments
    assert!(kwargs.is_none());

    let py = args.py();

    let wrapped: Borrowed<'_, '_, PyFunction> = args.get_borrowed_item(0)?.cast()?;
    let wrapped_args = args.get_borrowed_item(2)?.cast()?;
    let wrapped_kwargs = args.get_borrowed_item(3)?.cast()?;

    // todo: continue on failed extraction; cache the signatures
    let sign = extract_signature(wrapped)?;

    match sign.bind(
        py,
        &wrapped_args,
        wrapped_kwargs,
        wrapped.get_defaults(),
        wrapped.get_kw_defaults(),
    ) {
        Ok(_) => {}
        Err(e) => error!("failed to bind arguments to {wrapped:?} parameters: {e}"),
    }

    // todo: pass signature args
    let span = span(py, Level::INFO, kwargs);
    let guard = span.as_ref().map(|x| x.enter());
    let res = wrapped.call(wrapped_args, Some(&wrapped_kwargs))?.unbind();
    drop(guard);
    Ok(res)
}
