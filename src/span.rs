...
use crate::{
    ...
    leak::{Leaker, VecLeaker, leak_or_get_kwargs},
...
struct SpanAction<'a, 'py> {
    kwargs: Option<&'a Bound<'py, PyDict>>,
}

impl<'a, 'py> CallsiteAction for SpanAction<'a, 'py> {
    ...
    fn with_fields_and_values(
        ...
        let (fields, values) = leak_or_get_kwargs(None, self.kwargs);
...
pub(crate) fn span(py: Python, level: Level, kwargs: Option<&Bound<'_, PyDict>>) -> Option<Span> {
    ...
    callsite::do_action(py, level, SpanAction { kwargs })
}
